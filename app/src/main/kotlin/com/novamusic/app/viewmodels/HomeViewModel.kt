...
import com.novamusic.app.innertube.models.filterExplicit
import com.novamusic.app.innertube.models.filterVideo
...
import com.novamusic.app.db.entities.*
...
import com.novamusic.app.utils.credentialFlow
...
class HomeViewModel @Inject constructor(
    ...
    val allItemsMetadata = combine(
        ...
        database.allSongs()
    ) { libraryMeta, downloads, allSongs ->
        ...
        // New-style local-file downloads aren't in the Media3 downloads map, so
        // mark songs downloaded by the app as completed for the UI badges.
        allSongs.filter { it.song.isDownloadedByApp() }.forEach { song ->
            val current = metadataMap[song.id] ?: ItemMetadata()
            metadataMap[song.id] = current.copy(downloadState = Download.STATE_COMPLETED)
        }
    ...
    init {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            context.dataStore.credentialFlow(InnerTubeCookieKey)
                .collect { cookie ->
