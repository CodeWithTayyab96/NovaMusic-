...
Hardening, resilience, and release pipeline completion:

### Hardening and stability
- Parameterized the dynamic SQL in the Room migrations that update existing rows
  (`Migration5To6`, `Migration6To7`, `Migration13To14`) instead of building those
  statements with string interpolation.
- Removed an unused coroutine launch in the download helper.
- Added a coroutine-cancellation check before retrying in the InnerTube layer.
- Bounded the on-disk canvas cache with conservative defaults and added cleanup/
  eviction behavior so the cache does not grow without limit.

### Release pipeline
- Released from a freshly prepared repository snapshot with version tracking
  aligned to `CHANGELOG.md`.
- Verified that the repository already includes a GitHub Actions release workflow
  (`release-build.yml`) that builds the signed universal release APK and publishes
  the GitHub Release from the same successful run.

### Known follow-ups
- The Together (collaborative listening) feature remains disabled until NovaMusic
  hosts its own endpoint.
- Discord RPC app-icon and default activity-button URLs are repointed to this
  repository's assets.
