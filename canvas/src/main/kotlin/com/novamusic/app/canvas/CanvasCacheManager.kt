...
package com.novamusic.app.canvas
...
object CanvasCacheManager {
    ...
    // Cache limits - conservative defaults
    private const val DEFAULT_MAX_CACHE_SIZE_BYTES = 50L * 1024 * 1024 // 50 MB
    private const val DEFAULT_MAX_CACHE_FILES = 100
    private const val CLEANUP_BATCH_SIZE = 10
    ...
    private var maxCacheSizeBytes = DEFAULT_MAX_CACHE_SIZE_BYTES
    private var maxCacheFiles = DEFAULT_MAX_CACHE_FILES
    ...
    /**
     * Sets the maximum cache size in bytes. Defaults to 50 MB.
     * Call this before caching any canvases to configure limits.
     */
    fun setMaxCacheSize(maxBytes: Long, maxFiles: Int = DEFAULT_MAX_CACHE_FILES) {
        maxCacheSizeBytes = maxBytes.coerceAtLeast(1L * 1024 * 1024) // Minimum 1 MB
        maxCacheFiles = maxFiles.coerceAtLeast(10)
    }

    /**
     * Returns the current cache size in bytes.
     */
    private fun currentCacheSizeBytes(): Long {
        var size = 0L
    ...
    suspend fun cacheCanvas(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Cache indefinitely but subject to LRU cleanup when limits exceeded
                ...
                // Trigger cleanup if limits exceeded
                cleanupIfNeeded()
    ...
    /**
     * Cleans up old cache entries if limits are exceeded.
     * Uses LRU-style cleanup based on cachedAt timestamp.
     */
    private suspend fun cleanupIfNeeded() {
        if (memoryCache.isEmpty()) return

        val currentSize = currentCacheSizeBytes()
        val currentCount = memoryCache.size

        if (currentSize <= maxCacheSizeBytes && currentCount <= maxCacheFiles) return

        Timber.d("🎵 CanvasCache - Cleanup needed: size=$currentSize/$maxCacheSizeBytes, files=$currentCount/$maxCacheFiles")

        withContext(Dispatchers.IO) {
            // Sort by cachedAt (oldest first) for LRU cleanup
            val sortedEntries = memoryCache.values.sortedBy { it.cachedAt }

            var toDelete = mutableListOf<CachedCanvas>()
            var deletedSize = 0L
            var deletedCount = 0

            // First pass: delete enough to get under both limits
            for (entry in sortedEntries) {
                if (currentSize - deletedSize <= maxCacheSizeBytes &&
                    currentCount - deletedCount <= maxCacheFiles) {
                    break
                }

                toDelete.add(entry)
                deletedSize += File(entry.filePath).length()
                deletedCount++

                if (toDelete.size >= CLEANUP_BATCH_SIZE) break
            }

            toDelete.forEach { entry ->
                removeFromCacheInternal(entry.id)
            }

            Timber.d("🎵 CanvasCache - Cleanup completed: deleted $deletedCount files, ${deletedSize / 1024} KB")
        }
    }

    /**
     * Internal removal without triggering additional cleanup.
     */
    private fun removeFromCacheInternal(id: String) {
    ...
    private fun loadMetadata() {
        try {
            ...
            if (file.exists()) {
                ...
                Timber.d("🎵 CanvasCache - Cargando ${list.size} entradas del metadata")

                // Load all valid entries (those that haven't expired)
                val now = System.currentTimeMillis()
                list.filter { cached ->
                    cached.expiresAt == Long.MAX_VALUE || cached.expiresAt > now
                }.forEach { cached ->
                    val videoFile = File(cached.filePath)
                    if (videoFile.exists()) {
                ...
                Timber.d("🎵 CanvasCache - Cargados ${memoryCache.size} metadatos válidos")

                // If there were invalid entries, rewrite metadata to clean up
                if (list.size != memoryCache.size) {
                    saveMetadata()
                }
            }
        } catch (e: Exception) {
